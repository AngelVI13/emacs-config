...
#+END_SRC
** Centered-window
Centers text on window (i.e. applies padding to left and right so that the display shows nicely)
...
** C/C++ style
   ...
   #+END_SRC
** Patch style
Removes windows EOL chars in files containing both UNIX and DOS line endings
...
*** Elpy
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     ;; Fix trailing whitespaces on save
...
#+END_SRC
** Line movement
Move to lines logically and not visually, if down arror is pressed it will move to next
line and ignore any wrapped lines in the way
...
** Toggle between horizontal and vertical layout: C-x |
** Move to start/end of defun (function/class/method etc): C-M-a / C-M-e
